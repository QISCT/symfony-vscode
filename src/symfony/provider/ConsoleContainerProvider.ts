...
export class ConsoleContainerProvider implements ContainerProviderInterface {
  ...
  provideServiceDefinitions(): Promise<ServiceDefinition[]> {
    return this._executeCommand<ServiceDefinition>(
      ...
      (obj) => {
        ...
        if (obj.definitions !== undefined) {
          Object.keys(obj.definitions).forEach((key) => {
            collection[key] = new ServiceDefinition(
              key,
              obj.definitions[key].class,
              obj.definitions[key].public,
              null
            );
          });
        }
        if (obj.aliases !== undefined) {
          Object.keys(obj.aliases).forEach((key) => {
            ...
            let className = collection[alias]
              ? collection[alias].className
              : null;
            collection[key] = new ServiceDefinition(
              key,
              className,
              obj.aliases[key].public,
              alias
            );
          });
        }
        Object.keys(collection).forEach((key) => {
          if (
            !this._matchServicesFilters(
              collection[key].id,
              collection[key].className
            )
          ) {
  ...
  private _matchServicesFilters(
    ...
    return Object.keys(filters).some((filter) => {
      if (
        filters[filter] === "id" &&
        serviceId != null &&
        serviceId.match(new RegExp(filter))
      ) {
        return true;
      } else if (
        filters[filter] === "class" &&
        serviceClassName != null &&
        serviceClassName.match(new RegExp(filter))
      ) {
        return true;
      }
      return false;
  ...
  private _matchRoutesFilters(routeId: string, routePath: string): boolean {
    ...
    return Object.keys(filters).some((filter) => {
      if (
        filters[filter] === "id" &&
        routeId != null &&
        routeId.match(new RegExp(filter))
      ) {
        return true;
      } else if (
        filters[filter] === "path" &&
        routePath != null &&
        routePath.match(new RegExp(filter))
      ) {
        return true;
      }
      return false;
  ...
  private _matchParametersFilters(parameterId: string): boolean {
    ...
    return filters.some((filter) => {
      return (
        parameterId != null && parameterId.match(new RegExp(filter)) !== null
      );
    });
  }
}
